...
| 현재 단계 | MVP 프론트엔드 시연본 완성, Supabase 연동 전 단계 |
...
| 공개 여부 | Public |
| 기본 브랜치 | `master` |
| 최근 커밋 | `feat: 랜딩페이지 추가 및 6개 메뉴 골격 구성 (개발중 배지 포함)` |
| 커밋 수 | 2개 |
...
- [ ] **상담일지 골격 페이지 추가**  
  현재 6개 메뉴 중 상담일지는 랜딩 소개 카드만 있고 앱 내 페이지가 없음.  
  `page-counseling` div 추가, 네비게이션에 메뉴 항목 추가 필요.
